...
vlc_module_begin ()
    ...
    add_string( "subsdec-encoding", "",
                ENCODING_TEXT, ENCODING_LONGTEXT, 0 )
        change_string_list( ppsz_encodings, ppsz_encoding_names )
    add_int( "subsdec-autodetect-utf8", 1,
              AUTODETECT_UTF8_TEXT, AUTODETECT_UTF8_LONGTEXT, 0 )
    add_int( "subsdec-formatted", 1, FORMAT_TEXT, FORMAT_LONGTEXT,
                 0 )
vlc_module_end ()
#endif
...
static int OpenDecoder( vlc_object_t *p_this )
{
    ...
    else
    {
        ...
        if (var_Inheritint (p_dec, "subsdec-autodetect-utf8"))
        {
...
static subpicture_t *ParseText( decoder_t *p_dec, block_t *p_block )
{
    ...
    if( var_Inheritint( p_dec, "subsdec-formatted" ) )
...
static char *StripTags( char *psz_subtitle )
{
    ...
    while( *psz_subtitle )
    {
        if( *psz_subtitle == '<' )
        {
            // if( myStrncmp( psz_subtitle, "<br/>", 5 ) == 0 )
            if( myStrncmp( psz_subtitle, "<br/>", 5 ) == 0 )
                *psz_text++ = '\n';
        ...
        {
            if( !myStrncmp( psz_subtitle, "&lt;", 4 ))
            {
                *psz_text++ = '<';
                psz_subtitle += strcspn( psz_subtitle, ";" );
            }
            else if( !myStrncmp( psz_subtitle, "&gt;", 4 ))
            {
                *psz_text++ = '>';
                psz_subtitle += strcspn( psz_subtitle, ";" );
            }
            else if( !myStrncmp( psz_subtitle, "&amp;", 5 ))
            {
                *psz_text++ = '&';
                psz_subtitle += strcspn( psz_subtitle, ";" );
            }
            else if( !myStrncmp( psz_subtitle, "&quot;", 6 ))
            {
                *psz_text++ = '\"';
                psz_subtitle += strcspn( psz_subtitle, ";" );
            }
...
static void HtmlNPut( char **ppsz_html, const char *psz_text, int i_max )
{
    ...
    if( psz_html != NULL )
    {
        strncpy( psz_html + i_offset, psz_text, i_len );
...
static char *CreateHtmlSubtitle( int *pi_align, char *psz_subtitle )
{
    ...
    while( *psz_subtitle )
    {
        ...
        {
            if( !myStrncmp( psz_subtitle, "<br/>", 5 ))
            {
                HtmlCopy( &psz_html, &psz_subtitle, "<br/>" );
            }
            else if( !myStrncmp( psz_subtitle, "<b>", 3 ) )
            {
                HtmlCopy( &psz_html, &psz_subtitle, "<b>" );
                HtmlPut( &psz_tag, "b" );
            }
            else if( !myStrncmp( psz_subtitle, "<i>", 3 ) )
            {
                HtmlCopy( &psz_html, &psz_subtitle, "<i>" );
                HtmlPut( &psz_tag, "i" );
            }
            else if( !myStrncmp( psz_subtitle, "<u>", 3 ) )
            {
                HtmlCopy( &psz_html, &psz_subtitle, "<u>" );
                HtmlPut( &psz_tag, "u" );
            }
            else if( !myStrncmp( psz_subtitle, "<s>", 3 ) )
            {
                HtmlCopy( &psz_html, &psz_subtitle, "<s>" );
                HtmlPut( &psz_tag, "s" );
            }
            else if( !myStrncmp( psz_subtitle, "<font ", 6 ))
            {
                ...
                while( *psz_subtitle != '>' )
                {
                    ...
                    for( k=0; psz_attribs[ k ]; k++ )
                    {
                        ...
                        if( !myStrncmp( psz_subtitle, psz_attribs[k], i_len ) )
                        {
                            /* */
            ...
            else if( !myStrncmp( psz_subtitle, "</", 2 ))
            {
                int   b_match     = 0;
                int   b_ignore    = 0;
                ...
                if( i_len >= 0 )
                {
                    ...
                    {
                    case 'b':
                        b_match = !myStrncmp( psz_subtitle, "</b>", 4 );
                        i_len   = 4;
                        break;
                    case 'i':
                        b_match = !myStrncmp( psz_subtitle, "</i>", 4 );
                        i_len   = 4;
                        break;
                    case 'u':
                        b_match = !myStrncmp( psz_subtitle, "</u>", 4 );
                        i_len   = 4;
                        break;
                    case 's':
                        b_match = !myStrncmp( psz_subtitle, "</s>", 4 );
                        i_len   = 4;
                        break;
                    case 'f':
                        b_match = !myStrncmp( psz_subtitle, "</font>", 7 );
                        i_len   = 7;
                        break;
                    case 'I':
                        i_len = strcspn( psz_subtitle, ">" );
                 ...
                 strchr( psz_subtitle, '}' ) )
        {
            ...
            if( !b_has_align &&
                !myStrncmp( psz_subtitle, "{\\an", 4 ) && psz_subtitle[4] >= '1' && psz_subtitle[4] <= '9' && psz_subtitle[5] == '}' )
            {
