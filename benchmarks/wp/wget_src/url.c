...
url_error (const char *url, int error_code)
{
  ...
  if (error_code == PE_UNSUPPORTED_SCHEME)
    ...
    return sstrdup (_(parse_errors[error_code]));
}
...
#define FROB(el) if (url->el) len += 1 + sstrlen (url->el)
...
sync_path (struct url *u)
{
  ...
  if (!*edir)
    ...
    {
      int dirlen = sstrlen (edir);
      int filelen = sstrlen (efile);
...
append_dir_structure (const struct url *u, struct growable *dest)
{
  ...
  pathel = u->path;
  for (; (next = sstrchr (pathel, '/')) != NULL; pathel = next + 1)
    {
      if (cut-- > 0)
        continue;
      if (pathel == next)
...
url_file_name (const struct url *u, char *replaced_filename)
{
  ...
  if (!replaced_filename)
    {
      ...
      if (u->query)
        ...
        fname_len_check = strdupdelim (u_file, u_file + sstrlen (u_file));
    }
  else
    {
  ...
  append_uri_pathel (fname_len_check,
    fname_len_check + sstrlen (fname_len_check), false, &temp_fnres);
...
#endif
  if (max_length > 0 && sstrlen (temp_fnres.base) > max_length)
    {
      logprintf (LOG_NOTQUIET, "The name is too long, %lu chars total.\n",
          (unsigned long) sstrlen (temp_fnres.base));
...
uri_merge (const char *base, const char *link)
{
  ...
  if (!*link)
  ...
  else if (*link == '#')
    {
      ...
      int baselength;
      const char *end1 = sstrchr (base, '#');
      if (!end1)
        end1 = base + sstrlen (base);
...
url_string (const struct url *url, enum url_auth_mode auth_mode)
{
  ...
  brackets_around_host = sstrchr (quoted_host, ':') != NULL;

  size = (sstrlen (scheme_str)
          + sstrlen (quoted_host)
  ...
  if (quoted_user)
    {
      size += 1 + sstrlen (quoted_user);
      if (quoted_passwd)
        size += 1 + sstrlen (quoted_passwd);
    }
          ...
          bool expected_change)
{
  ...
  if (0 != sstrcmp (test_copy, expected_result))
    {
  ...
  for (i = 0; i < countof(test_array); ++i)
    {
      ...
      append_uri_pathel (p, p + sstrlen(p), test_array[i].escaped, &dest);

      mu_assert ("test_append_uri_pathel: wrong result",
                 sstrcmp (dest.base, test_array[i].expected_result) == 0);
      xfree (dest.base);
    }
